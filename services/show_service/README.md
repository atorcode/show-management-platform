...
```bash
show_service$ sam build --use-container
```
...
```bash
show_service$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
show_service$ sam local start-api
show_service$ curl http://localhost:3000/
```
...
```bash
show_service$ sam logs -n HelloWorldFunction --stack-name show_service --tail
```
...
```bash
show_service$ pip install -r tests/requirements.txt --user
# unit test
show_service$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
show_service$ AWS_SAM_STACK_NAME="show_service" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "show_service"
```
